        ...
        VStack(spacing: 0) {
            ...
            case .playlists, .collaborators:
                ScrollView {
                    ...
                    // Inside content(for artist:) method, add before the last closing brace
                    .onAppear {
                        // Set up notification observers
                        let addSongObserver = NotificationCenter.default.addObserver(
                            forName: Notification.Name("AddSongTapped"),
                            object: nil,
                            queue: .main
                        ) { [self] _ in
                            showAddSong = true
                        ...
                        let addPlaylistObserver = NotificationCenter.default.addObserver(
                            forName: Notification.Name("AddPlaylistTapped"),
                            object: nil,
                            queue: .main
                        ) { [self] _ in
                        ...
                        // Store the observers so we can remove them later
                        NotificationCenter.default.addObserver(forName: UIScene.didDisconnectNotification, object: nil, queue: .main) { _ in
                            NotificationCenter.default.removeObserver(addSongObserver)
                            NotificationCenter.default.removeObserver(addPlaylistObserver)
                        }
                    }
                }
            }
    ...
    var body: some View {
        if let artist {
            VStack(spacing: 0) {
                ...
                // Section header with add button and edit button
                HStack {
                    Text("Songs")
                        .font(.title3)
                        .fontWeight(.semibold)
                    ...
                    Button {
                        NotificationCenter.default.post(
                            name: Notification.Name("AddSongTapped"),
                            object: nil
                        )
                    } label: {
...
private struct PlaylistsList: View {
    ...
    var body: some View {
        if let artist {
            VStack(spacing: 0) {
                // Header with count, add button and edit button
                HStack {
                    ...
                    Button {
                        NotificationCenter.default.post(
                            name: Notification.Name("AddPlaylistTapped"),
                            object: nil
                        )
                    } label: {
                        Image(systemName: "plus.circle.fill")
